<script>
	...
	let email = '';
	let password = '';
	const hasAuto=location.href.indexOf('auto')>-1;
	...
	const signInHandler = async () => {
		const sessionUser = await userSignIn('felix@harmam.com','123123').catch((error) => {
			toast.error(error);
			return null;
		});
...
{#if loaded&&!hasAuto}
	...
	<div class=" bg-white dark:bg-gray-950 min-h-screen w-full flex justify-center font-mona">
			<div class="flex justify-center items-center">
				<img
					crossorigin="anonymous"
					src="{WEBUI_BASE_URL}/static/HARMAN_logo.png"
					alt="logo"
					style="width:450px;padding-right:30px;border-right:2px solid gray"
				/>
			</div>
